...
//    	$today = strtotime("-1 day");
    ...
    public function changeTableVal(){  
		...
		if($table == 'goods_sku' && $field = 'sku_price'){
            $goods_id = M($table)->where("$id_name = $id_value")->getfield('goods_id');
            (new \app\admin\model\Goods())->carPrice($goods_id);
        }
        if($table == 'goods_sku' && $field = 'sku_count'){
            $goods_id = M($table)->where("$id_name = $id_value")->getfield('goods_id');
            (new \app\admin\model\Goods())->carCount($goods_id);
        }
    }	
