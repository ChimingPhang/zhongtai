...
class Report extends Base{
    ...
    public function _initialize(){
        ...
        if(I('start_time')){
            $begin = I('start_time');
            $end = I('end_time',date('Y-m-d', strtotime('+1 days')));
        }else{
    ...
    public function saleTop(){
        ...
        $sql = "SELECT og.goods_name,og.goods_sn,sum(og.goods_num) AS sale_num,sum(og.goods_num * og.goods_price) AS sale_amount FROM tp_order_goods{$this->select_year} og JOIN  tp_order{$this->select_year} o ON o.order_id = og.order_id";
        ...
        $sql .=" WHERE o.order_status = 3 and o.add_time between {$this->begin} and {$this->end}";
...
//        dump(Db::name('order')->getLastSql());die;
...
//        }
        ...
        $classList = $GoodsLogic->getSortClass();  //获取排好序的品牌列表
