...
//		dump($where);
		...
		$store_list = M('dealers')->getField('id,name');
		$this->assign('store_list',$store_list);
		$this->assign('goods_list',$goods_list);
	...
	public function refund_info(){
		if(IS_POST){
			...
			M('return_goods')->where(['id'=>$data['id']])->save(['refund_mark'=>$data['refund_mark']]);
		}
