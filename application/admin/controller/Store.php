	...
	public function store_del(){
		...
		if($store_id > 1){
			...
			if ($storeGoodsCount > 0) {
				...
				M('store_apply')->where("user_id", $store['user_id'])->delete();
...
//			$this->error('操作失败');
	...
	public function reopen_list()
	{
        ...
        if (IS_POST){
            ...
            $grade_id ? $where['grade_id'] = $grade_id : false;       //店铺等级
    ...
    public function reopen_save(){
        ...
        $res = $StoreReopenModel->where($save_where)->save(['re_state'=>2]);
        if ($res === false)$this->error('审核失败！');
        $this->success('审核成功！');
    }
