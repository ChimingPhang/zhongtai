...
//    	$strTable .= '<td style="text-align:center;font-size:12px;" width="*">会员等级</td>';
    ...
    public function address(){
        $uid = I('get.id');
        $lists = M('user_address')->where(array('user_id'=>$uid,'status'=>1))->select();
     ...
     *
     * @Autoh: 胡宝强
     * Date: 2018/7/15 14:21
     */
    public function integral_log(){
        $user_id = I('id','',intval);
        if(empty($user_id)){
            $this->error('用户id不能为空',U('User/index'));
            exit;
        }
        if(!is_numeric($user_id)){
            $this->error('用户id必须为纯数字',U('User/index'));
            exit;
        }
        $count = M('integral_log')->where(['user_id'=>$user_id])->count();
        $page = new Page($count,10);
        $list = M('integral_log')->where(['user_id'=>$user_id])->order('create_time desc')->limit($page->firstRow.','.$page->listRows)->select();
        $this->assign('page',$page->show());
