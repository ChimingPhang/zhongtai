...
class GoodsLogic extends Model
{
     ...
     * @return  mix
     */
    public function goods_sku_list($goods_id)
    {
        global $goods_category, $goods_category2;
        $sql = "SELECT * FROM  __PREFIX__goods_sku WHERE `goods_id` = {$goods_id} ORDER BY parent_id , sort_order ASC";
        $goods_category = DB::query($sql);
        $goods_category = convert_arr_key($goods_category, 'id');
    ...
    /**
     * 改变或者添加属性时 需要修改他下面的 parent_id_path  和 level
     * @global type $cat_list 所有商品分类
     * @param type $parent_id_path 指定的id
     * @return 返回数组 Description
     */
    public function refresh_sku($id)
    {
        $GoodsCategory = M("GoodsSku"); // 实例化User对象
        $cat = $GoodsCategory->where("id = $id")->find(); // 找出他自己
        // 刚新增的分类先把它的值重置一下
        if($cat['parent_id_path'] == '')
        {
            ($cat['parent_id'] == 0) && $GoodsCategory->execute("UPDATE __PREFIX__goods_sku set  parent_id_path = '0_$id', level = 1 where id = $id"); // 如果是一级分类
            $GoodsCategory->execute("UPDATE __PREFIX__goods_sku AS a ,__PREFIX__goods_sku AS b SET a.parent_id_path = CONCAT_WS('_',b.parent_id_path,'$id'),a.level = (b.level+1) WHERE a.parent_id=b.id AND a.id = $id");
            $cat = $GoodsCategory->where("id = $id")->find(); // 从新找出他自己
        }
        ...
        $replace_level = $cat['level'] - ($parent_cat['level'] + 1); // 看看他 相比原来的等级 升级了多少  ($parent_cat['level'] + 1) 他老爸等级加一 就是他现在要改的等级
        $replace_str = $parent_cat['parent_id_path'].'_'.$id;
        $GoodsCategory->execute("UPDATE `__PREFIX__goods_sku` SET parent_id_path = REPLACE(parent_id_path,'{$cat['parent_id_path']}','$replace_str'), level = (level - $replace_level) WHERE  parent_id_path LIKE '{$cat['parent_id_path']}%'");
        if($cat['level'] == 3){
            (new Goods())->carPrice($cat['goods_id']);
        }elseif($cat['level'] == 4){
            (new Goods())->carCount($cat['goods_id']);
        }
    }
    /**
     ...
     * @param type $cat_id
     */
    function find_parent_sku($cat_id,$goods_id)
    {
        if($cat_id == null)
            return array();

        $cat_list =  M('goods_sku')->where(array("goods_id" => $goods_id))->getField('id,parent_id,level');
        $cat_level_arr[$cat_list[$cat_id]['level']] = $cat_id;
    ...
    {
        $brandList = S('getSortClass',$brandList);
        if(!empty($brandList))
            return $brandList;
        $brandList =  M("AccessoriesCategory")->where(array('status' => ['<>','-1']))->select();
        $nameList = array();
        foreach($brandList as $k => $v)
        {
            $name = getFirstCharter($v['name']) .'  --   '. $v['name']; // 前面加上拼音首字母
...
//         dump(M('Service')->getlastsql());die;

        $keySpecGoodsPrice = M('SpecGoodsPrice')->where("store_id = $store_id and goods_id = $goods_id")->getField('key,key_name,price,cost_price,store_count,weight,integral_price,integral');//规格项

        $str = "<table class='table table-bordered' id='spec_input_tab'>";
        $str .="<tr>";
        if($spec_arr2){
            // 显示第一行的数据
            foreach ($clo_name as $k1 => $v)
            {
            ...
            foreach ($spec_arr2 as $k => $v)
            {
                ...
                foreach($v as $k2 => $v2)
                {
                    $str .="<td class='tabletd' >{$specItem[$v2][item]}</td>";
...
//                    $item_key_name[$v2] = $spec[$specItem[$v2]['spec_id']].':'.$specItem[$v2]['item'];
                ...
                $keySpecGoodsPrice[$item_key][cost_price] ? false : $keySpecGoodsPrice[$item_key][cost_price] = 0; // 价格默认为0
                $keySpecGoodsPrice[$item_key][store_count] ? false : $keySpecGoodsPrice[$item_key][store_count] = 0; //库存默认为0
                $keySpecGoodsPrice[$item_key][weight] ? false : $keySpecGoodsPrice[$item_key][weight] = 0; //库存默认为0
                ...
                $str .="<td>￥<input name='item[$item_key][integral_price]' value='{$keySpecGoodsPrice[$item_key][integral_price]}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' /></td>";
                $str .="<td>￥<input name='item[$item_key][integral]' value='{$keySpecGoodsPrice[$item_key][integral]}' onkeyup='this.value=this.value.replace(/[^\d]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d]/g,\"\")' /></td>";
                ...
                $str .="<td><input name='item[$item_key][weight]' value='{$keySpecGoodsPrice[$item_key][weight]}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d]/g,\"\")'/>g</td>";
                $str .="</tr>";
            }
        }
        $str .= "</table>";
