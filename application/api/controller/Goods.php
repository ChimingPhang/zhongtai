...
class Goods extends Base {
    ...
    {
        //检测必传参数
        (!is_numeric($type = I('type', 0)) || !in_array($type, [1,2]) ) &&  $this->errorMsg(2002, 'type');//必传
...
//        self::$redis->Zincrby('hot_words', 1, $title);
    ...
    public function carInfo()
    {
        ...
        $field = "goods_id,goods_name,equity_content,label,equity_desc,goods_remark,price,deposit_price,store_count,sales_sum,goods_content,integral,exchange_integral,integral_money as integrals_moneys";
        $data = $Goods->GoodsList($this->page, 1, $where, [], 1, $field);
        $data->equity_desc = str_replace("
", "<br/>", $data->equity_desc);
        $data->equity_desc = str_replace(" ", "&nbsp;", $data->equity_desc);
        $data->equity_content = str_replace("
", "<br/>", $data->equity_content);
    ...
    public function partsInfo()
    {
        ...
        //商品详情
        $where['goods_id'] = $goods_id;
        $field = "goods_id,goods_name,goods_remark,price,label,store_count,sales_sum,goods_content,integral,moren_integral";
