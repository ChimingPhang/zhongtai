...
class OrderLogic extends Model
{
     ...
     * @Author   XD
     * @DateTime 2018-07-20T13:58:19+0800
     * @return   [type]                   [description]
     */
    public function checkOrder($rec_id){
        if(empty($rec_id)) return ['status'=>-1,'msg'=>'参数错误'];
        $where['rec_id'] = $rec_id;
        $order_id = M('order_goods')->where($where)->getField('order_id');
        $res = M('order_goods')->where($where)->save(['is_comment'=>1]);
        $where['is_comment'] = 1;
        $check = M('order_goods')->where($where)->count();
        if(!$check){
            M('order')->where(['order_id'=>$order_id])->save(['is_comment'=>1,'order_status'=>4]);
        }
        return $res;
    }
	...
	public function cancel_order($user_id,$order_id){
		...
		if($reduce == 1 || empty($reduce)){
			if($order['type'] == 1){
				$this->alterReturnGoodsInventoryCar($order);
			}else{
        ...
        if($data['type'] < 2){
            ...
            if($order['order_amount']>0){
                if($order['order_amount']>$order['shipping_price']){
                    $data['refund_money'] = round($rate*($order['order_amount'] - $order['shipping_price']),2); //退款金额
                    $data['refund_deposit'] = $rate*$order['user_money'];
                }else{
                    $data['refund_deposit'] = round($rate*($order['user_money'] - $order['shipping_price'])-$integralDeductionMoney,2);//该退余额支付部分
                }
            }else{
		...
		foreach($order_goods as $key=>$val){
			if(!empty($val['spec_key'])){ // 先到规格表里面扣除数量 再重新刷新一个 这件商品的总数量
                ...
                if($specGoodsPrice){
                    ...
                    M('goodsSku')->where(['goods_id' => $val['goods_id'], 'parent_id_path' => $val['spec_key']])->setInc('store_count', $val['goods_num']);
                }
			}else{
	 ...
	 * @param $rec_id|订单商品表id 如果有只返还订单某个商品的库存,没有返还整个订单
	 */
	public function alterReturnGoodsInventoryCar($order, $rec_id='')
	{
		if($rec_id){
			$orderGoodsWhere['rec_id'] = $rec_id;
			$retunn_info = Db::name('return_goods')->where($orderGoodsWhere)->select(); //查找购买数量和购买规格
...
//					M('goodsSku')->where(['goods_id' => $val['goods_id'], 'parent_id_path' => $val['spec_key']])->setInc('store_count', $val['goods_num']);
//				}
				$specGoodsPrice = M('goods_sku')->where(['goods_id' => $val['goods_id'],'parent_id_path'=>$val['spec_key']])->find();

				M('goods_sku')->where(['goods_id' => $val['goods_id'],'parent_id_path'=>$val['spec_key']])->setInc('sku_count', $val['goods_num']);
			}else{
				M('goods')->where(['goods_id' => $val['goods_id']])->setInc('store_count', $val['goods_num']);//没有规格则增加商品库存
			}
			update_stock_log($order['user_id'], $val['goods_num'], $val, $order['order_sn']);//库存日志
