...
//            }
            //还要判断这个订单是不是纯积分支付的，如果是纯积分支付不用走微信退款
...
//            }
    ...
    function delOrder($order_id){
        $select_year = getTabByOrderId($order_id);
        $a = M('order'.$select_year)->where(array('order_id'=>$order_id))->update(['deleted'=>1]);
    ...
    /**
     * 车型的计算
     * 取消车型的订单后改变库存，根据不同的规格，商品活动修改对应的库存
     * @param $order|订单
