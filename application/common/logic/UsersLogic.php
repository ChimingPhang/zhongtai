...
class UsersLogic extends Model
{
    ...
    public function login($username, $password)
    {
        ...
        $user = Db::name('users')->where(['mobile'=>$username,'deleted'=>0])->find();
        if (!$user) {
     ...
     * @return array
     */
    private function getThirdUser($openid, $unionid, $oauth, $oauth_child = '')
    {
        $user = [];
        $thirdUser = Db::name('oauth_users')->where(['openid' => $openid, 'oauth' => $oauth])->find();
        if ( ! $thirdUser) {
            if ($unionid) {
                $thirdUser = Db::name('oauth_users')->where(['unionid' => $unionid])->find();
            }
            if ($thirdUser) {
                //补充其他第三方登录方式
        ...
        if ($thirdUser) {
            ...
            if ( ! $user) {
                Db::name('oauth_users')->where(['openid' => $openid, 'oauth' => $oauth])->delete();//删除残留数据
            }
        }
    ...
    public function thirdLogin($data = array())
    {
        ...
        $user = $this->getThirdUser($openid, $unionid, $oauth, $data['oauth_child']);
        if (empty($user)) {
            //账户不存在 注册一个
            $map['password'] = '';
            $map['openid'] = $openid;
    ...
    public function thirdLogin_new($data = array())
    {
        ...
        $user = $this->getThirdUser($data['openid'], $data['unionid'], $data['oauth'], $data['oauth_child']);
        if( ! $user){
