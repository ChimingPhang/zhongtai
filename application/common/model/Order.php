...
class Order extends Model
{
    ...
    {
        $data_status_arr = C('ORDER_STATUS_DESCS');
        // 货到付款
        if ($data['pay_code'] == 'cod') {
            ...
            if ($data['pay_status'] == 1 && in_array($data['order_status'], array(0, 1)) && $data['shipping_status'] != 1) {
                if ($data['prom_type'] == 5) { //虚拟商品
                    ...
                    if($data['shipping_status'] == 0) return $data_status_arr['WAITSEND']; //'待发货',
                    if($data['shipping_status'] == 2) return $data_status_arr['WAITRECEIVE']; //'待收货',
                }
            }
        }
