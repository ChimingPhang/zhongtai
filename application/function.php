 ...
 * @return mixed
 */
function hanzi_sort($distributorList, $sort = 'asc')
{
    $sort = $sort == 'asc' ? SORT_ASC : SORT_DESC;
    $flag = [];
    foreach ($distributorList as $key => $value){
        $flag[] = $distributorList[$key]['name'] = mb_convert_encoding($value['name'], 'GB2312','UTF-8');
    }
    array_multisort($flag,$sort,$distributorList);
    foreach ($distributorList as $key => $value){
        $distributorList[$key]['name'] = iconv('GB2312', 'UTF-8',$value['name']);
    }
