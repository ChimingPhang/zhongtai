...
class Goods extends Base
{
    ...
    public function goodsInfo()
    {
        ...
        if (cookie('user_id')) {
            $users = Users::get(cookie('user_id'));
            $user = new User($users);
            $user->visitGoodsLog($goods);
        }
        $goods->save(['click_count' => $goods['click_count'] + 1]); //点击数+1
        $goods_attribute = Db::name('goods_attribute')->where(['attr_index'=>1])->getField('attr_id,attr_name'); // 查询属性
        $spec_goods_price = Db::name('spec_goods_price')->where('goods_id',$goods_id)->getField("key,item_id,price,store_count"); // 规格 对应 价格 库存表
        $filter_spec = $goodsLogic->get_spec($goods_id);//这里由于数据库设计的问题，优化不来。
        $look_goods_list = Db::name('goods')->field('goods_name,goods_id,shop_price')->where(['cat_id1'=>['neq',$goods['cat_id1']],'is_on_sale'=>1])->limit(12)->select();
