 ...
 * @return array
 */
function passwordForSeller($user_name,$mobile, $new_password, $is_update = true)
{
    $user = M('dealers')->where(['user_name'=>$user_name,'mobile'=>$mobile,'status'=>0,'deleted'=>1])->find();
    ...
    $row = M('dealers')->where(['user_name'=>$user_name,'mobile'=>$mobile])->update(array('password' => encrypt($new_password)));
    if (!$row) {
