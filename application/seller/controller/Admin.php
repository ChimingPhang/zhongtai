...
class Admin extends Base
{
    ...
    public function modify_pwd()
    {
        ...
        $info = D('dealers')->where(array('id' => STORE_ID))->find();
        if (!$info) {
    ...
    public function login()
    {
        ...
        if (IS_POST) {
            ...
            if (!empty($seller_name) && !empty($password)) {
                $seller = M($table)->where(array('user_name' => $seller_name,'deleted'=>1))->find();
                if ($seller) {
                	$store = M($table)->where(array('id'=>$seller['id'],'status'=>1))->select();
                	if(!$store) exit(json_encode(array('status' => 0, 'msg' => '经销商已关闭，请联系平台客服')));


                    $user_where = ['user_name'=>$seller_name,'password'=>encrypt($password)];
                    $user = M($table)->where($user_where)->find();
                    if ($user) {
...
//                        }
                        ...
                        session('store_id', $seller['id']);
                        M($table)->where(array('id' => $seller['id']))->save(array('last_login_time' => time()));
                        //sellerLog('经销商管理中心登录');
