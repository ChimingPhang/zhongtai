...
//			}
			M('return_goods')->where(array('id'=>$data['id'],'store_id'=>STORE_ID))->save($data);
			if($data['status'] == 1){
				logOrder($return_goods['order_id'], '同意了用户的退款申请', '同意退款', $return_goods['user_id'], 1);
			}else{
				logOrder($return_goods['order_id'], '拒绝了用户的退款申请', '拒绝退款', $return_goods['user_id'], 1);
			}
...
//			$data = I('post.');
//			$data['checktime'] = time();
//			$data['refund_mark'] = $data['remark'];
//			$data['is_receive'] = 1;
//			if($data['status'] == 1){
//				//同意退款 计算要退多少积分
//				$data['refund_integral'] = return_jifen($return_goods['id'],$return_goods['rec_id'],$return_goods['order_id']);
//				//$arr = M('return_goods')->where(array('id'=>$data['id'],'store_id'=>STORE_ID))->save($data);
//				$aa = M('return_goods')->where(array('id'=>$data['id'],'store_id'=>STORE_ID))->find();
//				logOrder($aa['order_id'], '同意了用户的退款申请', '同意退款', $aa['user_id'], 1);
//			}else{
//				$arr = M('return_goods')->where(array('id'=>$data['id'],'store_id'=>STORE_ID))->save($data);
//				$aa = M('return_goods')->where(array('id'=>$data['id'],'store_id'=>STORE_ID))->find();
//				logOrder($aa['order_id'], '取消了用户的退款申请', '取消退款', $aa['user_id'], 1);
//				if($arr){
//					$this->success('操作成功!',U('Service/refund_list'));
//				}else{
//					$this->success('操作失败!',U('Service/refund_list'));
//				}
//			}
...
//			if($data['status'] == 1){
//
//				$orderLogic = new \app\common\logic\SaleLogic();
//				$final_return_info = $orderLogic->getFinalReturnInfo($data['id']); //重新计算一次要退款
//				$orderLogic->alterReturnGoodsInventoryCar($final_return_info,$final_return_info['rec_id']);//退货后改变库存
//
//				//$CommonOrderLogic = new \app\common\logic\SaleLogic();
//				//$CommonOrderLogic->alterReturnGoodsInventoryCar($final_return_info,$final_return_info['rec_id']);//退货后改变库存
