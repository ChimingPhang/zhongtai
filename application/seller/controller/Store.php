...
//        }
        $apply = M('dealers')->where("id", STORE_ID)->find();
    ...
    public function store_reopen()
    {
        ...
        $info['re_end_time']=$re_end_time = $store_reopen[0]['re_end_time'];  //到期时间
        $info['earlier']=$earlier= 30; //可提前申请时间
        $info['start_apply_time'] = $re_end_time-($earlier*60*60*24);  //继续续期开始时间
        if($info['start_apply_time'] < time() && $store_reopen[0]['re_state']==2){    //是否可续签时间
