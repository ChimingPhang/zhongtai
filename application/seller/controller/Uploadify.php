...
class Uploadify extends Base{
    ...
    public function delupload(){
        ...
        if($action=='del' && !empty($filename) && file_exists($filename)){
        	...
        	if($fileArr[3] != $store_id) return false;
            $size = getimagesize($filename);
            $filetype = explode('/',$size['mime']);
            if($filetype[0]!='image'){
                exit;
            }*/
            $finfo    = finfo_open(FILEINFO_MIME);  //获取文件mime类型
    ...
    public function index(){
    	...
    	$CONFIG2 = json_decode(str_replace('FILE_URL',C('FILE_URL'),preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents("./public/plugins/Ueditor/php/config.json"))), true);
    	$action = $_GET['action'];
