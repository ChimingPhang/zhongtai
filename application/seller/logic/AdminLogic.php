    ...
    public function alterAdminPassword($data=''){
        ...
        if ($data['seller_id'] == $seller['id']) {
            ...
            if ($old_password == $new_password) {
                ...
                if (M('dealers')->where(array('id' => $data['seller_id'], 'password' => encrypt($old_password)))->count() > 0) {
                    $r = M('dealers')->where(array('id' => $data['seller_id']))->save(array('password' => encrypt($new_password)));
                    if ($r !== false) {
                        //M('seller')->where(['seller_id'=>$data['seller_id']])->save(array('enabled' => $data['enabled'],'group_id'=>$data['group_id']));
                        return ['status' =>1, 'msg'=>"密码修改成功", 'url'=>U('Index/index')];
                    } else {
