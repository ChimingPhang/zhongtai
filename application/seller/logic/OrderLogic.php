...
class OrderLogic
{
	...
	public function getOrderGoodss($order_id,$is_send =''){
		if($is_send){
			$where=" and o.is_send < $is_send";
		}
		...
		$res = M('order')->alias('o')
			->join('tp_order_goods g','o.order_id = g.order_id','right')
			->join('tp_goods gg','gg.goods_id = g.goods_id','left')
			->where($where)
			->field('g.*,gg.*,o.dj,(g.goods_num * g.member_goods_price) AS goods_total')
			->select();
		//$sql = "SELECT g.*,o.*,(o.goods_num * o.member_goods_price) AS goods_total FROM __PREFIX__order_goods{$select_year} o ".
		//	"LEFT JOIN __PREFIX__goods g ON o.goods_id = g.goods_id WHERE o.order_id = :order_id ".$where;
		//$res = Db::query($sql,['order_id'=>$order_id]);
...
//		}
    ...
    public function orderProcessHandle($order_id,$act,$store_id = 0){
    	...
    	switch ($act){
			...
			case 'pay_all': //订单全部付完
				...
				$updata['is_all_pay_time'] = time();
				$arr['is_shouhuo'] = 1;
				$arr['is_send'] = 1;
				M('order_goods')->where(['order_id'=>$order_id,'store_id'=>$store_id])->save($arr);
				break;
    		case 'invalid': //作废订单
    ...
    public function getShowStatus($order)
    {
        ...
        if ($order['pay_status'] == 0 && $order['order_status'] == 0 && $order['pay_code'] != 'cod') {
            ...
            $show_status = 4;
        }else if($order['pay_status'] == 1 && $order['order_status'] == 3 && $order['is_all_pay'] == 2){
			$show_status = 6;
		}
        return $show_status;
    }
    ...
    public function deliveryHandle($data,$store_id){
		...
		Db::name('order')->where('order_id',$data['order_id'])->update(['shipping_code'=>$data['shipping_code'],'shipping_name'=>$data['shipping_name']]);
		$is_delivery = 0;
