...
class Auth extends Controller {
    //定义变量appId和appSecret
//    private static $appId,$appSecret;
...
//        self::$appSecret = Config::get('app_secret');
    }
    ...
    public function _initialize()
    {
        ...
        if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger')) {
            $this->weixin_config = M('wx_user')->cache(true)->find(); //获取微信配置
...
//                $wxuser = $this->getUserOpenId(); //授权获取openid以及微信用户信息
    ...
    public function getBaseInfo() {
        ...
        $url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" . $this->weixin_config['appid'] . "&redirect_uri=" . $redirect_uri . "&response_type=code&scope=snsapi_base&state=123#wechat_redirect";
        header('location:' . $url);
    }
